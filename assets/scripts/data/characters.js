export const characters = [
  {
    code: 0,
    urlImage: "assets/images/jon_snow_image.jpg",
    alt: "Jon Snow, o herdeiro de Winterfell",
    nameCharacters: "Jon Snow",
    house: "Stark / Targaryen",
    origin: "Winterfell",
    quote: "Eu não quero isso. Eu nunca quis.",
    description:
      "Jon Snow, filho bastardo de Eddard Stark, cresce em Winterfell, sempre lidando com o peso de sua origem. Embora inicialmente tenha dificuldades com sua identidade, ele se prova um líder nato. Desde sua entrada na Patrulha da Noite até sua batalha contra os Caminhantes Brancos, sua jornada o coloca no centro das maiores batalhas e decisões. Jon também luta para equilibrar suas crenças de honra e justiça com as difíceis realidades do poder. Ao longo da série, ele é forçado a fazer escolhas difíceis, desde sua aliança com o Povo Livre até seu papel na batalha contra os mortos-vivos. Descobre-se, então, que ele é, na verdade, Aegon Targaryen, com um direito legítimo ao Trono de Ferro. Mesmo com essa revelação, ele continua leal aos Starks, preferindo proteger os Sete Reinos a reivindicar o trono. Jon é um símbolo de sacrifício, resiliência e liderança, colocando sempre o dever à frente de seus interesses pessoais.",
    backgroundImage: "assets/images/jon_snow_background.jpg",
    gallery: [
      "assets/images/jon_snow_1.jpg",
      "assets/images/jon_snow_2.jpg",
      "assets/images/jon_snow_3.jpg",
    ],
  },

  {
    code: 1,
    urlImage: "assets/images/tyrion_lannister_image.jpg",
    alt: "Tyrion Lannister, o estrategista sagaz de Casterly Rock",
    nameCharacters: "Tyrion Lannister",
    origin: "Casterly Rock",
    house: "Lannister",
    quote: "Eu bebo e sei das coisas.",
    description:
      "Tyrion Lannister, o filho mais novo de Tywin Lannister, foi sempre tratado com desprezo por causa de sua estatura e por ser considerado uma desonra à família. No entanto, sua mente brilhante o colocou como um dos personagens mais estratégicos e astutos de Westeros. Desde cedo, Tyrion aprendeu a usar a inteligência e o sarcasmo para superar os preconceitos que sofria. Apesar do desprezo de seu pai e irmã, Cersei, ele manteve sua lealdade à Casa Lannister, mas suas ações sempre buscaram a justiça e a moralidade. Após fugir de um julgamento injusto, Tyrion encontrou refúgio com Daenerys Targaryen, onde se tornou um de seus principais conselheiros. Ao longo de sua jornada, Tyrion enfrentou questões de identidade, pertencimento e sobrevivência em um mundo que muitas vezes o via como inferior. Ele lutou tanto contra os horrores de sua própria família quanto pelas pessoas comuns de Westeros, tornando-se um personagem amado por sua sagacidade, resiliência e capacidade de ver além dos jogos de poder.",
    backgroundImage: "assets/images/tyrion_background.jpg",
    gallery: [
      "assets/images/tyrion_1.jpg",
      "assets/images/tyrion_2.jpg",
      "assets/images/tyrion_3.jpg",
    ],
  },

  {
    code: 2,
    urlImage: "assets/images/daenerys_targaryen_image.jpg",
    alt: "Daenerys Targaryen, a última da casa Targaryen",
    nameCharacters: "Daenerys Targaryen",
    origin: "Dragonstone",
    house: "Targaryen",
    quote:
      "Eu sou Daenerys Targaryen e eu vou pegar o que é meu com sangue e fogo.",
    description:
      "Daenerys Targaryen, também conhecida como a Mãe dos Dragões, foi uma das últimas descendentes da Casa Targaryen, que governou Westeros por séculos antes de ser derrubada. Nascida durante uma tempestade em Dragonstone e criada no exílio, ela passou grande parte de sua vida fugindo com seu irmão Viserys. Após ser casada com Khal Drogo, líder dos Dothraki, ela começou sua ascensão como líder poderosa, libertando cidades escravizadas em Essos e conquistando a lealdade de milhões. Suas ações eram guiadas por um forte senso de justiça e desejo de quebrar sistemas de opressão, mas seu caminho ao trono também trouxe uma jornada complexa de poder, perda e ambição. Os dragões que ela trouxe de volta ao mundo simbolizavam não apenas sua linhagem ancestral, mas sua visão de si mesma como a legítima herdeira do Trono de Ferro. Ao longo de sua jornada, Daenerys enfrentou desafios imensos, desde traições até a decisão de sacrificar seu próprio senso de moralidade em busca do que acreditava ser o bem maior. Seu destino trágico foi marcado por sua transformação de uma jovem oprimida para uma figura quase divina de poder absoluto.",
    backgroundImage: "assets/images/daenerys_background.jpg",
    gallery: [
      "assets/images/daenerys_1.jpg",
      "assets/images/daenerys_2.jpg",
      "assets/images/daenerys_3.jpg",
    ],
  },

  {
    code: 3,
    urlImage: "assets/images/cersei_lannister_image.jpg",
    alt: "Cersei Lannister, a Rainha Regente de Westeros",
    nameCharacters: "Cersei Lannister",
    origin: "Casterly Rock",
    house: "Lannister",
    quote: "Quando você joga o jogo dos tronos, você ganha ou morre.",
    description:
      "Cersei Lannister é uma das figuras mais intrigantes e implacáveis de Westeros. Nascida em Casterly Rock, filha de Tywin Lannister, ela cresceu cercada de poder e privilégio. Através de seu casamento com o Rei Robert Baratheon, ela se tornou a Rainha de Westeros e, após a morte de Robert, consolidou sua posição como Rainha Regente e, mais tarde, como Rainha de fato. Ao longo de sua vida, Cersei demonstrou uma habilidade única para manipular aqueles ao seu redor, utilizando sua inteligência e crueldade para eliminar seus inimigos e proteger seus filhos a qualquer custo. Sua trajetória é marcada por uma busca incansável por poder e vingança, o que a levou a momentos de grande vitória, mas também a profundas tragédias pessoais. No final, sua obsessão pelo poder e seu desejo de manter o controle sobre Westeros a conduzem a um destino inevitavelmente trágico. Cersei representa o lado mais sombrio do jogo dos tronos, onde a ambição e o medo de perder o poder superam qualquer noção de moralidade ou compaixão.",
    backgroundImage: "assets/images/cersei_background.jpg",
    gallery: [
      "assets/images/cersei_1.jpg",
      "assets/images/cersei_2.jpg",
      "assets/images/cersei_3.jpg",
    ],
  },

  {
    code: 4,
    urlImage: "assets/images/arya_stark_image.jpg",
    alt: "Arya Stark, a jovem guerreira de Winterfell",
    nameCharacters: "Arya Stark",
    origin: "Winterfell",
    house: "Stark",
    quote: "Ninguém é mais forte do que quem não tem nada a perder.",
    description:
      "Arya Stark é uma figura emblemática em 'Game of Thrones', caracterizada por sua resiliência e bravura. Desde jovem, ela sempre se sentiu deslocada nas normas da sociedade, preferindo a espada ao bordado. Após o brutal assassinato de seu pai, Eddard Stark, e a subsequente destruição de sua família, Arya inicia uma jornada de transformação e vingança. Através de experiências traumáticas, ela desenvolve habilidades impressionantes, desde combate com espadas até o aprendizado de técnicas de furtividade sob a tutela dos Homens Sem Rosto em Braavos. Sua notável capacidade de adaptação e aprendizado, combinada com sua lista de vingança, a tornam uma das personagens mais complexas e empolgantes da série. Arya não é apenas uma guerreira; ela é uma sobrevivente que desafia os limites da lealdade e da identidade, sempre buscando justiça para sua família e para si mesma.",
    backgroundImage: "assets/images/arya_background.jpg",
    gallery: [
      "assets/images/arya_1.jpg",
      "assets/images/arya_2.jpg",
      "assets/images/arya_3.jpg",
    ],
  },

  {
    code: 5,
    urlImage: "assets/images/sansa_stark_image.jpg",
    alt: "Sansa Stark, a estrategista e sobrevivente de Winterfell",
    nameCharacters: "Sansa Stark",
    origin: "Winterfell",
    house: "Stark",
    quote: "A dor é um presente. A dor é o que nos torna mais fortes.",
    description:
      "Sansa Stark é uma das personagens mais fascinantes e complexas de 'Game of Thrones'. Desde o início da série, Sansa se apresenta como uma jovem ingênua, sonhando com um futuro de contos de fadas, mas sua jornada a leva a enfrentar as crueldades da vida real em Westeros. Ao longo das temporadas, Sansa passa por uma profunda transformação, de uma menina sonhadora para uma líder astuta e determinada. Sua experiência em Portão da Tempestade e em King's Landing a ensina sobre traição, manipulação e sobrevivência. Ao se unir à Casa Stark em Winterfell, ela demonstra sua resiliência e capacidade de se adaptar, utilizando a dor que sofreu como uma ferramenta para fortalecer sua posição. Sansa emerge como uma estrategista competente, pronta para reivindicar seu lugar como uma Stark e proteger sua família e seu lar.",
    backgroundImage: "assets/images/sansa_background.jpg",
    gallery: [
      "assets/images/sansa_1.jpg",
      "assets/images/sansa_2.jpg",
      "assets/images/sansa_3.jpg",
    ],
  },

  {
    code: 6,
    urlImage: "assets/images/brienne_of_tarth.jpg",
    alt: "Brienne de Tarth, a guerreira leal e destemida",
    nameCharacters: "Brienne de Tarth",
    origin: "Tarth",
    house: "Tarth",
    quote: "A honra é tudo que temos.",
    description:
      "Brienne de Tarth é uma das figuras mais emblemáticas de 'Game of Thrones', destacando-se não apenas como uma mulher guerreira em um mundo dominado por homens, mas também como um símbolo de lealdade e honra. Desde sua introdução, Brienne desafia as expectativas tradicionais de gênero, lutando com coragem e destreza, mesmo quando enfrentando o desprezo de muitos. Ao longo da série, sua lealdade à Casa Stark e ao seu juramento a Lady Stark a tornam uma personagem admirável. A busca de Brienne para provar seu valor e encontrar um propósito em um mundo caótico a transforma em uma heroína atípica, mostrando que a verdadeira força não reside apenas na habilidade de lutar, mas também na capacidade de permanecer fiel a seus princípios.",
    backgroundImage: "assets/images/brienne_background.jpg",
    gallery: [
      "assets/images/brienne_1.jpg",
      "assets/images/brienne_2.jpg",
      "assets/images/brienne_3.jpg",
    ],
  },

  {
    code: 7,
    urlImage: "assets/images/joffrey_baratheon.jpg",
    alt: "Joffrey Baratheon, o jovem rei cruel",
    nameCharacters: "Joffrey Baratheon",
    origin: "Porto Real",
    house: "Casa Baratheon",
    quote: "Eu sou o rei!",
    description:
      "Joffrey Baratheon, o primeiro filho de Cersei Lannister e Robert Baratheon, é um dos antagonistas mais odiados de 'Game of Thrones'. Desde jovem, Joffrey exibe um caráter cruel e sádico, demonstrando uma profunda necessidade de poder e controle. Como rei, ele governa com um punho de ferro, punindo qualquer um que desafie sua autoridade, e sua relação com a mãe, Cersei, molda sua percepção distorcida do mundo. Joffrey é famoso por sua incapacidade de lidar com a pressão do trono, suas ações brutais e sua tendência a buscar prazer na dor dos outros, o que o torna um símbolo do que há de pior na busca pelo poder em Westeros.",
    backgroundImage: "assets/images/joffrey_background.jpg",
    gallery: [
      "assets/images/joffrey_1.jpg",
      "assets/images/joffrey_2.jpg",
      "assets/images/joffrey_3.jpg",
    ],
  },

  {
    code: 8,
    urlImage: "assets/images/petyr_baelish.jpg",
    alt: "Petyr Baelish, o manipulador astuto",
    nameCharacters: "Petyr Baelish",
    origin: "Porto Real",
    house: "Casa Baelish",
    quote: "O caos não é uma poça. O caos é uma escada.",
    description:
      "Petyr Baelish, também conhecido como 'Littlefinger', é um dos personagens mais intrigantes e manipuladores de 'Game of Thrones'. Desde suas origens humildes, Petyr ascendeu nas fileiras da sociedade, utilizando seu intelecto aguçado e sua habilidade em manipular aqueles ao seu redor. Ele é conhecido por seu papel em várias conspirações que moldaram o destino de Westeros. Seu amor obsessivo por Catelyn Stark o leva a tomar decisões que, embora astutas, são frequentemente motivadas por interesses pessoais. Baelish é um mestre das intrigas e, por meio de suas ações, demonstra como o desejo de poder pode levar à destruição.",
    backgroundImage: "assets/images/petyr_background.jpg",
    gallery: [
      "assets/images/petyr_1.jpg",
      "assets/images/petyr_2.jpg",
      "assets/images/petyr_3.jpg",
    ],
  },

  {
    code: 9,
    urlImage: "assets/images/jaime_lannister.jpg",
    alt: "Jaime Lannister, o cavaleiro complexo e redentor",
    nameCharacters: "Jaime Lannister",
    origin: "Porto Real",
    house: "Casa Lannister",
    title: "Regicida",
    quote: "O homem que não tem nada a perder é o mais perigoso.",
    description:
      "Jaime Lannister, conhecido como Regicida, é um dos personagens mais complexos de 'Game of Thrones'. Inicialmente apresentado como um vilão arrogante, sua jornada ao longo da série revela um lado mais profundo e redentor. Membro da poderosa Casa Lannister, Jaime é um habilidoso espadachim e um cavaleiro fiel, embora suas lealdades sejam frequentemente testadas. Seu amor por Cersei e sua amizade com Tyrion o colocam em situações difíceis, desafiando suas crenças sobre honra e dever. Ao longo da série, Jaime passa por uma transformação significativa, confrontando suas escolhas e lutando para redimir seu passado.",
    backgroundImage: "assets/images/jaime_background.jpg",
    gallery: [
      "assets/images/jaime_1.jpg",
      "assets/images/jaime_2.jpg",
      "assets/images/jaime_3.jpg",
    ],
  },

  {
    code: 10,
    urlImage: "assets/images/robb_stark.jpg",
    alt: "Robb Stark, o jovem lorde de Winterfell",
    nameCharacters: "Robb Stark",
    origin: "Winterfell",
    house: "Casa Stark",
    title: "Rei do Norte",
    quote: "A guerra é um jogo de sorte.",
    description:
      "Robb Stark, o filho mais velho de Eddard Stark, é o jovem lorde de Winterfell e Rei do Norte. Ele é um líder carismático e justo, guiado pelos valores de honra e lealdade que sua família representa. Após a execução de seu pai, Robb assume a responsabilidade de proteger sua família e vingar a injustiça cometida. Ele se destaca como um estrategista militar, conquistando várias vitórias nas batalhas contra os Lannisters. No entanto, suas decisões e alianças políticas se revelam complicadas, levando a um destino trágico. Robb é um símbolo da luta pela justiça e do peso da responsabilidade que recai sobre os ombros de um líder.",
    backgroundImage: "assets/images/robb_background.jpg",
    gallery: [
      "assets/images/robb_1.jpg",
      "assets/images/robb_2.jpg",
      "assets/images/robb_3.jpg",
    ],
  },

  {
    code: 11,
    urlImage: "assets/images/melisandre.jpg",
    alt: "Melisandre, a Sacerdotisa Vermelha",
    nameCharacters: "Melisandre",
    origin: "Asshai",
    house: "Nenhuma",
    title: "Sacerdotisa Vermelha",
    quote: "A sombra é minha amiga.",
    description:
      "Melisandre, conhecida como a Sacerdotisa Vermelha, é uma misteriosa e poderosa feiticeira que serve ao Deus do Fogo, R'hllor. Nascida em Asshai, ela possui habilidades sobrenaturais, incluindo a capacidade de ver o futuro em suas visões. Melisandre é uma figura enigmática, muitas vezes considerada uma manipuladora, que acredita firmemente que sua missão é levar a luz em tempos de escuridão. Ela desempenha um papel crucial na guerra contra os mortos, guiando e influenciando personagens como Stannis Baratheon e Jon Snow. Apesar de sua devoção ao seu deus, Melisandre é frequentemente vista com desconfiança devido aos métodos que emprega para alcançar seus objetivos.",
    backgroundImage: "assets/images/melisandre_background.jpg",
    gallery: [
      "assets/images/melisandre_1.jpg",
      "assets/images/melisandre_2.jpg",
      "assets/images/melisandre_3.jpg",
    ],
  },

  {
    code: 12,
    urlImage: "assets/images/sandor_clegane.jpg",
    alt: "Sandor Clegane, o Cão de Caça",
    nameCharacters: "Sandor Clegane",
    origin: "Ferro",
    house: "Casa Clegane",
    title: "Cão de Caça",
    quote: "Eu sou o Cão.",
    description:
      "Sandor Clegane, também conhecido como o Cão, é um personagem complexo que serve como guarda-costas e fiel escudeiro de Joffrey Baratheon. Ele é marcado por sua brutalidade e um profundo desprezo pela hipocrisia da nobreza. Apesar de sua aparência intimidadora e seu comportamento violento, Sandor tem um lado mais humano que se revela ao longo da história, especialmente em suas interações com Arya Stark. A cicatriz que ele carrega no rosto, resultado de um incêndio causado por seu irmão, Gregor Clegane, também molda sua personalidade e sua relação com a dor e a violência. Sandor é uma figura trágica que busca um propósito em meio à crueldade do mundo em que vive.",
    backgroundImage: "assets/images/sandor_clegane_background.jpg",
    gallery: [
      "assets/images/sandor_clegane_1.jpg",
      "assets/images/sandor_clegane_2.jpg",
      "assets/images/sandor_clegane_3.jpg",
    ],
  },

  {
    code: 13,
    urlImage: "assets/images/bran_stark.jpg",
    alt: "Bran Stark, o Corvo de Três Olhos",
    nameCharacters: "Bran Stark",
    origin: "Winterfell",
    house: "Casa Stark",
    title: "O Corvo de Três Olhos",
    quote: "O passado não pode ser mudado.",
    description:
      "Bran Stark é um personagem central na saga de 'As Crônicas de Gelo e Fogo'. Após sofrer uma queda que o deixa paraplégico, Bran descobre que possui habilidades místicas, incluindo a capacidade de ver eventos passados e futuros através dos olhos de outros. Ao longo da série, ele se transforma no Corvo de Três Olhos, um guardião do conhecimento e da história de Westeros. Sua jornada é marcada pela busca de identidade e compreensão do seu papel no destino dos Sete Reinos. Bran é um símbolo de esperança e conexão com o passado, trazendo uma nova perspectiva sobre a luta pelo poder em Westeros.",
    backgroundImage: "assets/images/bran_stark_background.jpg",
    gallery: [
      "assets/images/bran_stark_1.jpg",
      "assets/images/bran_stark_2.jpg",
      "assets/images/bran_stark_3.jpg",
    ],
  },

  {
    code: 14,
    urlImage: "assets/images/stannis_baratheon.jpg",
    alt: "Stannis Baratheon, o Rei do Fogo e do Gelo",
    nameCharacters: "Stannis Baratheon",
    origin: "Dragonstone",
    house: "Casa Baratheon",
    title: "Rei de Westeros",
    quote: "A vitória é o único desejo que conheço.",
    description:
      "Stannis Baratheon é um dos personagens mais complexos e determinados da saga 'As Crônicas de Gelo e Fogo'. Irmão mais novo de Robert Baratheon, Stannis é conhecido por sua integridade e senso de dever. Ele se considera o legítimo rei de Westeros, insistindo que a coroa deve pertencer a ele devido ao direito de primogenitura. Sua aliança com a misteriosa Melisandre e sua devoção ao Senhor da Luz o levam a tomar decisões sombrias e muitas vezes controversas. Stannis é um comandante militar astuto, mas sua obsessão pela legitimidade e pela vitória o coloca em conflito com aliados e inimigos, culminando em tragédias que definem seu destino.",
    backgroundImage: "assets/images/stannis_baratheon_background.jpg",
    gallery: [
      "assets/images/stannis_baratheon_1.jpg",
      "assets/images/stannis_baratheon_2.jpg",
      "assets/images/stannis_baratheon_3.jpg",
    ],
  },

  {
    code: 15,
    urlImage: "assets/images/renly_baratheon_image.jpg",
    alt: "Renly Baratheon, o Rei da Luz Verde",
    nameCharacters: "Renly Baratheon",
    origin: "Storm's End",
    house: "Casa Baratheon",
    title: "Rei de Westeros",
    quote: "Os homens devem ser governados; não são capazes de se governar.",
    description:
      "Renly Baratheon é o mais jovem dos irmãos Baratheon e uma figura carismática e encantadora em 'As Crônicas de Gelo e Fogo'. Ele é conhecido por seu forte senso de justiça e seu desejo de paz, contrastando com a brutalidade de seu irmão Stannis e a ambição de Joffrey. Renly busca o trono de ferro, afirmando seu direito ao reino após a morte de seu irmão Robert. Seu carisma atrai muitos seguidores, incluindo a famosa Brienne de Tarth. Renly se destaca por sua conexão com os outros e seu idealismo, mas sua busca pelo poder é marcada por desafios e intrigas que culminam em sua trágica morte.",
    backgroundImage: "assets/images/renly_baratheon_background.jpg",
    gallery: [
      "assets/images/renly_baratheon_1.jpg",
      "assets/images/renly_baratheon_2.jpg",
      "assets/images/renly_baratheon_3.jpg",
    ],
  },

  {
    code: 16,
    urlImage: "assets/images/theon_greyjoy.jpg",
    alt: "Theon Greyjoy, o Príncipe de Ferro",
    nameCharacters: "Theon Greyjoy",
    origin: "Ilhas de Ferro",
    house: "Casa Greyjoy",
    title: "Príncipe de Ferro",
    quote: "Eu sou o que sou, e sou o que sou porque fiz o que fiz.",
    description:
      "Theon Greyjoy é o filho de Balon Greyjoy e um dos personagens mais complexos da série. Criado como um 'refém' na Casa Stark, Theon luta para encontrar sua identidade entre os Stark e os Greyjoy. Sua lealdade é testada quando ele tenta reconquistar seu lar, Pike, e isso leva a uma série de escolhas trágicas. Theon é marcado por sua busca por aceitação e seu desejo de se provar, o que o leva a situações sombrias, incluindo traição e arrependimento. Sua jornada é uma mistura de bravura, desespero e busca por redenção, especialmente após os eventos traumáticos que enfrenta.",
    backgroundImage: "assets/images/theon_greyjoy_background.jpg",
    gallery: [
      "assets/images/theon_greyjoy_1.jpg",
      "assets/images/theon_greyjoy_2.jpg",
      "assets/images/theon_greyjoy_3.jpg",
    ],
  },

  {
    code: 17,
    urlImage: "assets/images/tywin_lannister.jpg",
    alt: "Tywin Lannister, o Patriarca da Casa Lannister",
    nameCharacters: "Tywin Lannister",
    origin: "Rochedo Casterly",
    house: "Casa Lannister",
    title: "Mestre da Casa Lannister, Mão do Rei",
    quote: "Um leão não se preocupa com a opinião das ovelhas.",
    description:
      "Tywin Lannister é um dos personagens mais poderosos e temidos em 'Game of Thrones'. Como patriarca da Casa Lannister, ele é conhecido por sua astúcia política e suas táticas implacáveis para manter o controle e o poder. Seu caráter forte é exemplificado por sua habilidade em manipular eventos para o benefício de sua família, mesmo que isso signifique recorrer a medidas brutais. Tywin tem uma relação complicada com seus filhos, especialmente com Tyrion, a quem despreza, e Cersei, cuja ambição ele frequentemente tenta moldar. Através de sua influência e liderança, Tywin se torna uma figura central nas guerras pelo Trono de Ferro.",
    backgroundImage: "assets/images/tywin_lannister_background.jpg",
    gallery: [
      "assets/images/tywin_lannister_1.jpg",
      "assets/images/tywin_lannister_2.jpg",
      "assets/images/tywin_lannister_3.jpg",
    ],
  },

  {
    code: 18,
    urlImage: "assets/images/davos_seaworth.jpg",
    alt: "Davos Seaworth, o Cavaleiro das Cebolas",
    nameCharacters: "Davos Seaworth",
    origin: "Fangorn",
    house: "N/A",
    title: "Cavaleiro das Cebolas, Mão do Rei",
    quote: "A honra é uma coisa complicada.",
    description:
      "Davos Seaworth, conhecido como o 'Cavaleiro das Cebolas', é um personagem de grande integridade e lealdade. Ele começou sua vida como um contrabandista, mas se destacou por sua coragem e sensatez. Davos serve como conselheiro leal a Stannis Baratheon, trazendo uma perspectiva pragmática e muitas vezes humilde para as intrigas políticas de Westeros. Apesar de seu passado criminoso, ele é um homem de princípios e um defensor do que é certo. Sua lealdade a Stannis e sua relação com Jon Snow destacam seu papel essencial nas histórias de redenção e honra em 'Game of Thrones'.",
    backgroundImage: "assets/images/davos_seaworth_background.jpg",
    gallery: [
      "assets/images/davos_seaworth_1.jpg",
      "assets/images/davos_seaworth_2.jpg",
      "assets/images/davos_seaworth_3.jpg",
    ],
  },

  {
    code: 19,
    urlImage: "assets/images/ygritte.jpg",
    alt: "Ygritte, a mulher do Povo Livre",
    nameCharacters: "Ygritte",
    origin: "Povo Livre",
    house: "N/A",
    title: "Guerreira e Arqueira",
    quote: "Você sabe nada, Jon Snow.",
    description:
      "Ygritte é uma figura icônica do Povo Livre, conhecida por sua determinação e habilidades excepcionais como arqueira. Ela tem um espírito indomável e uma forte conexão com a natureza, vivendo além da Muralha. Seu relacionamento com Jon Snow é central na narrativa, simbolizando o conflito entre dever e amor. Com sua personalidade forte e visão de mundo única, Ygritte desafia as convenções de Westeros e se torna um símbolo de resistência e paixão.",
    backgroundImage: "assets/images/ygritte_background.jpg",
    gallery: [
      "assets/images/ygritte_1.jpg",
      "assets/images/ygritte_2.jpg",
      "assets/images/ygritte_3.jpg",
    ],
  },

  {
    code: 20,
    urlImage: "assets/images/margaery_tyrell.jpg",
    alt: "Margaery Tyrell, a astuta e carismática",
    nameCharacters: "Margaery Tyrell",
    origin: "Casa Tyrell de Alto Jardim",
    house: "Tyrell",
    title: "Rainha de Westeros",
    quote: "A bondade é uma forma de poder.",
    description:
      "Margaery Tyrell é uma personagem astuta e carismática da Casa Tyrell, conhecida por sua beleza e inteligência. Desde jovem, ela é ensinada a manipular os poderes de Westeros, buscando sempre aumentar sua influência. Margaery se casa com vários reis em uma busca por poder, incluindo Renly Baratheon e Joffrey Baratheon. Seu caráter é complexo, combinando um forte senso de justiça e empatia por aqueles que são menos afortunados, o que a torna uma figura carismática e estratégica nas intrigas políticas de Westeros.",
    backgroundImage: "assets/images/margaery_tyrell_background.jpg",
    gallery: [
      "assets/images/margaery_tyrell_1.jpg",
      "assets/images/margaery_tyrell_2.jpg",
      "assets/images/margaery_tyrell_3.jpg",
    ],
  },

  {
    code: 21,
    urlImage: "assets/images/tormund_giantsbane.jpg",
    alt: "Tormund Giantsbane, o feroz guerreiro dos Livres",
    nameCharacters: "Tormund Giantsbane",
    origin: "Os Livres, além da Muralha",
    house: "N/A",
    title: "Líder dos Livres",
    quote:
      "Eu não sou um gigante, sou apenas um homem que se recusa a ser domado.",
    description:
      "Tormund Giantsbane é um feroz guerreiro e líder dos Livres, conhecido por sua coragem e lealdade. Ele se destaca por sua aparência imponente e seu espírito indomável, sendo um dos poucos que desafiam os costumes e regras do mundo dos homens. Tormund se torna um aliado importante de Jon Snow na luta contra os Caminhantes Brancos, demonstrando que a unidade entre diferentes povos é crucial para a sobrevivência. Ele também é conhecido por seu senso de humor e por ser uma figura carismática, conquistando a lealdade e o respeito de seus companheiros.",
    backgroundImage: "assets/images/tormund_giantsbane_background.jpg",
    gallery: [
      "assets/images/tormund_giantsbane_1.jpg",
      "assets/images/tormund_giantsbane_2.jpg",
      "assets/images/tormund_giantsbane_3.jpg",
    ],
  },

  {
    code: 22,
    urlImage: "assets/images/ned_stark.jpg",
    alt: "Ned Stark, o Senhor de Winterfell e Warden do Norte",
    nameCharacters: "Eddard 'Ned' Stark",
    origin: "Winterfell, no Norte de Westeros",
    house: "Stark",
    title: "Senhor de Winterfell, Warden do Norte",
    quote: "A primeira lição: o que acontece quando você trai a sua família.",
    description:
      "Eddard Stark, conhecido como Ned, é o leal Senhor de Winterfell e Warden do Norte. Um homem de honra e integridade, Ned é conhecido por seu forte senso de justiça e por sempre colocar a família acima de tudo. Ele é um pai dedicado e um líder respeitado, que luta para proteger seu lar e os seus entes queridos. Ao longo da série, Ned se vê envolvido em uma teia de intrigas políticas quando é chamado para se tornar Mão do Rei. Sua morte prematura deixou uma marca indelével no reino, e seu legado de honra continua a influenciar os eventos em Westeros.",
    backgroundImage: "assets/images/ned_stark_background.jpg",
    gallery: [
      "assets/images/ned_stark_1.jpg",
      "assets/images/ned_stark_2.jpg",
      "assets/images/ned_stark_3.jpg",
    ],
  },

  {
    code: 23,
    urlImage: "assets/images/missandei.jpg",
    alt: "Missandei, a conselheira sábia e intérprete de Daenerys Targaryen",
    nameCharacters: "Missandei",
    origin: "Astapor, uma cidade em Slaver's Bay",
    house: "N/A",
    title: "Conselheira e intérprete de Daenerys Targaryen",
    quote: "Dracarys.",
    description:
      "Missandei é uma personagem forte e inteligente, originalmente escravizada em Astapor. Ela se torna a conselheira e intérprete de Daenerys Targaryen, usando seu vasto conhecimento dos costumes e da língua dos escravos para ajudar a conquistadora. Com uma lealdade inabalável, Missandei desempenha um papel fundamental na estratégia de Daenerys e na luta pela libertação dos povos oprimidos. Sua história é uma jornada de liberdade e empoderamento, enquanto ela navega pelas complexidades da política e da guerra em Westeros.",
    backgroundImage: "assets/images/missandei_background.jpg",
    gallery: [
      "assets/images/missandei_1.jpg",
      "assets/images/missandei_2.jpg",
      "assets/images/missandei_3.jpg",
    ],
  },

  {
    code: 24,
    urlImage: "assets/images/jorah_mormont.jpg",
    alt: "Sor Jorah Mormont, o leal cavaleiro e protetor de Daenerys Targaryen",
    nameCharacters: "Sor Jorah Mormont",
    origin: "Ilha de Bear, Westeros",
    house: "Mormont",
    title: "Cavaleiro e Conselheiro",
    quote: "A vida é uma luta constante. A luta é tudo o que temos.",
    description:
      "Sor Jorah Mormont é um cavaleiro exilado de Westeros que se torna leal a Daenerys Targaryen. Inicialmente motivado por seu próprio desejo de redenção, Jorah se transforma em um dos aliados mais dedicados de Daenerys, usando suas habilidades de combate e sua experiência política para ajudá-la em sua jornada. Sua história é marcada por lealdade, sacrifício e a busca por um propósito, enquanto ele navega pelas complexidades de seu amor não correspondido por Daenerys e seu desejo de protegê-la a qualquer custo.",
    backgroundImage: "assets/images/sor_jorah_background.jpg",
    gallery: [
      "assets/images/sor_jorah_1.jpg",
      "assets/images/sor_jorah_2.jpg",
      "assets/images/sor_jorah_3.jpg",
    ],
  },

  {
    code: 25,
    urlImage: "assets/images/khal_drogo.jpg",
    alt: "Khal Drogo, o poderoso líder dos Dothraki",
    nameCharacters: "Khal Drogo",
    origin: "Cultura Dothraki, Essos",
    house: "Dothraki",
    title: "Khal",
    quote: "Se você não pode ser o melhor, seja o mais temido.",
    description:
      "Khal Drogo é um dos mais poderosos líderes Dothraki e um guerreiro temido. Ele é conhecido por sua força imbatível em batalha e sua habilidade como líder. Drogo se casou com Daenerys Targaryen, unindo seus destinos e ajudando-a a se tornar uma líder forte por direito próprio. Sua história é uma combinação de amor, poder e sacrifício, e ele desempenha um papel crucial na transformação de Daenerys de uma jovem inexperiente em uma rainha decidida. Sua jornada é marcada por lealdade à sua cultura e ao seu povo.",
    backgroundImage: "assets/images/khal_drogo_background.jpg",
    gallery: [
      "assets/images/khal_drogo_1.jpg",
      "assets/images/khal_drogo_2.jpg",
      "assets/images/khal_drogo_3.jpg",
    ],
  },

  {
    code: 26,
    urlImage: "assets/images/mirri_maz_duur.jpg",
    alt: "Mirri Maz Duur, a curandeira de Essos",
    nameCharacters: "Mirri Maz Duur",
    origin: "Lhaesh, Essos",
    house: "N/A",
    title: "Curandeira",
    quote: "O que foi feito não pode ser desfeito.",
    description:
      "Mirri Maz Duur é uma curandeira e sacerdotisa da cultura Lhazareen, conhecida por seus conhecimentos em medicina e magia. Ela é trazida a Khal Drogo para tratar suas feridas, mas suas habilidades são envoltas em mistério e tragédia. A personagem desempenha um papel crucial na trama, especialmente na transformação de Daenerys Targaryen. Mirri Maz Duur representa o conflito entre a ciência e a magia, e suas ações têm consequências significativas para a narrativa, refletindo temas de sacrifício e moralidade. Sua presença no enredo desafia os personagens a enfrentarem as complexidades da vida e da morte.",
    backgroundImage: "assets/images/mirri_maz_duur_background.jpg",
    gallery: [
      "assets/images/mirri_maz_duur_1.jpg",
      "assets/images/mirri_maz_duur_2.jpg",
      "assets/images/mirri_maz_duur_3.jpg",
    ],
  },

  {
    code: 27,
    urlImage: "assets/images/catelyn_stark.jpg",
    alt: "Catelyn Stark, a matriarca de Winterfell",
    nameCharacters: "Catelyn Stark",
    origin: "Winterfell, Westeros",
    house: "Stark",
    title: "Lady de Winterfell",
    quote: "A família é tudo.",
    description:
      "Catelyn Stark, née Tully, é a matriarca da Casa Stark e uma mulher forte e decidida. Ela é conhecida por seu amor inabalável pela família e sua lealdade aos Stark. Catelyn toma decisões difíceis para proteger seus filhos e sua casa, refletindo a complexidade das relações familiares em um mundo de intrigas e perigos. Sua jornada é marcada por tragédias e sacrifícios, e seu papel na história destaca a importância das mulheres nas narrativas de 'Game of Thrones'. Catelyn representa tanto a força quanto a vulnerabilidade da maternidade em meio ao caos.",
    backgroundImage: "assets/images/catelyn_stark_background.jpg",
    gallery: [
      "assets/images/catelyn_stark_1.jpg",
      "assets/images/catelyn_stark_2.jpg",
      "assets/images/catelyn_stark_3.jpg",
    ],
  },

  {
    code: 28,
    urlImage: "assets/images/hodor.jpg",
    alt: "Hodor, o leal protetor de Bran Stark",
    nameCharacters: "Hodor",
    origin: "Winterfell, Westeros",
    house: "Stark",
    title: "Guardião de Bran Stark",
    quote: "Hodor.",
    description:
      "Hodor, cujo nome verdadeiro é Wylis, é um personagem icônico da série, conhecido por sua lealdade e simplicidade. Ele serve como um protetor dedicado de Bran Stark, sempre disposto a arriscar sua vida por ele. Sua famosa frase, 'Hodor', é uma reminiscência de sua condição e uma expressão de seu amor e devoção. A história de Hodor é trágica e cheia de emoção, culminando em um sacrifício heroico que ressoa profundamente com os fãs da série. Ele simboliza a pureza e a bondade em um mundo frequentemente repleto de traições e violência.",
    backgroundImage: "assets/images/hodor_background.jpg",
    gallery: [
      "assets/images/hodor_1.jpg",
      "assets/images/hodor_2.jpg",
      "assets/images/hodor_3.jpg",
    ],
  },

  {
    code: 29,
    urlImage: "assets/images/ramsay_bolton.jpg",
    alt: "Ramsay Bolton, o cruel e impiedoso senhor de Winterfell",
    nameCharacters: "Ramsay Bolton",
    origin: "Winterfell, Westeros",
    house: "Bolton",
    title: "Senhor de Winterfell",
    quote: "Se você não pode ser bom, seja o melhor.",
    description:
      "Ramsay Bolton é um dos vilões mais temidos da série, conhecido por sua crueldade e sadismo. Como o filho bastardo de Roose Bolton, ele se destaca pela brutalidade e falta de compaixão, tornando-se um verdadeiro representante da casa Bolton. Ele é famoso por suas táticas tortuosas e por seu prazer em infligir dor e sofrimento aos outros, o que faz dele um dos antagonistas mais memoráveis. Ramsay busca o poder a qualquer custo, e sua relação com Theon Greyjoy exemplifica sua natureza manipuladora e cruel. Sua história culmina em uma luta pelo controle de Winterfell, onde seu destino se entrelaça com a de outros personagens em um enredo cheio de traições e reviravoltas.",
    backgroundImage: "assets/images/ramsay_bolton_background.jpg",
    gallery: [
      "assets/images/ramsay_1.jpg",
      "assets/images/ramsay_2.jpg",
      "assets/images/ramsay_3.jpg",
    ],
  },

  {
    code: 30,
    urlImage: "assets/images/jaqen_hghar.jpg",
    alt: "Jaqen H'ghar, o misterioso assassino de Braavos",
    nameCharacters: "Jaqen H'ghar",
    origin: "Braavos, Westeros",
    house: "Sem Casa",
    title: "Homem Sem Face",
    quote: "Um homem deve saber quando morrer.",
    description:
      "Jaqen H'ghar é um dos personagens mais enigmáticos de 'Game of Thrones', conhecido por sua habilidade de mudar de aparência e sua ligação com a Casa de Preto e Branco. Ele é um assassino treinado e membro da guilda de assassinos de Braavos, que serve a uma entidade conhecida como 'A Many-Faced God'. Durante a série, Jaqen se torna uma figura crucial para Arya Stark, ajudando-a a entender o caminho do assassinato e a importância de deixar para trás sua identidade anterior. Sua personalidade calma e suas habilidades extraordinárias fazem dele um personagem fascinante, simbolizando a complexidade da moralidade no mundo de Westeros.",
    backgroundImage: "assets/images/jaqen_hghar_background.jpg",
    gallery: [
      "assets/images/jaqen_1.jpg",
      "assets/images/jaqen_2.jpg",
      "assets/images/jaqen_3.jpg",
    ],
  },

  {
    code: 31,
    urlImage: "assets/images/samwell_tarly.jpg",
    alt: "Samwell Tarly, o leal amigo de Jon Snow e aprendiz de mestre",
    nameCharacters: "Samwell Tarly",
    origin: "Horn Hill, Westeros",
    house: "Casa Tarly",
    title: "Mestre",
    quote: "Eu sou Samwell Tarly, e eu sou um homem.",
    description:
      "Samwell Tarly é um personagem que representa a coragem e a determinação em face da adversidade. Ele é inicialmente apresentado como um jovem tímido e inseguro, mas ao longo da série, ele evolui em um combatente astuto e um valioso aliado. Sam é conhecido por sua inteligência e amor pela leitura, e é um dos poucos personagens que não busca poder ou glória, mas sim o conhecimento e a proteção dos que ama. Sua amizade com Jon Snow e seu papel como aprendiz de mestre na Muralha são fundamentais para a narrativa, especialmente em tempos de necessidade extrema.",
    backgroundImage: "assets/images/samwell_tarly_background.jpg",
    gallery: [
      "assets/images/samwell_1.jpg",
      "assets/images/samwell_2.jpg",
      "assets/images/samwell_3.jpg",
    ],
  },

  {
    code: 32,
    urlImage: "assets/images/lorde_varys.jpg",
    alt: "Lorde Varys, o Espião Mestre de Westeros",
    nameCharacters: "Lorde Varys",
    origin: "Cidade Livre de Lys",
    house: "Casa Varys (não tem uma casa em particular)",
    title: "Mestre dos Sussurros",
    quote:
      "O que é mais perigoso? Um homem sem medo ou um homem com uma agenda?",
    description:
      "Lorde Varys, conhecido como o 'Mestre dos Sussurros', é um intrigante e manipulador conselheiro na corte de Westeros. Nascido em Lys, sua história é marcada por segredos e espionagem. Ele serve os interesses do reino, acreditando que o bem do povo deve prevalecer, mesmo que suas táticas sejam frequentemente sombrias e imorais. Com uma habilidade única para ouvir e aprender, Varys tem uma rede de espiões que se estende por toda Westeros, o que o torna uma figura crucial nas lutas pelo poder. Sua lealdade é frequentemente questionada, mas seu profundo conhecimento da política e da dinâmica das casas nobres o torna um aliado valioso.",
    backgroundImage: "assets/images/lorde_varys_background.jpg",
    gallery: [
      "assets/images/varys_1.jpg",
      "assets/images/varys_2.jpg",
      "assets/images/varys_3.jpg",
    ],
  },

  {
    code: 33,
    urlImage: "assets/images/oberyn_martell.jpg",
    alt: "Oberyn Martell, o Príncipe da Dorne",
    nameCharacters: "Oberyn Martell",
    origin: "Dorne",
    house: "Casa Martell",
    title: "O Príncipe de Dorne",
    quote:
      "Eu sou a vingança, e a vingança é um prato que deve ser servido frio.",
    description:
      "Oberyn Martell, também conhecido como a 'Cobra Vermelha', é o Príncipe de Dorne e um dos personagens mais carismáticos de Westeros. Ele é famoso por sua habilidade com a lança, seu espírito livre e seu charme irresistível. Oberyn é profundamente leal à sua família e busca vingar a morte de sua irmã, Elia Martell, que foi brutalmente assassinada por Gregor Clegane. Sua personalidade audaciosa e destemida, combinada com suas habilidades marciais excepcionais, o tornam um jogador importante na luta pelo poder em Westeros, especialmente quando ele se envolve na disputa pela mão de sua filha. Oberyn é um homem que vive intensamente, buscando justiça e prazer em igual medida.",
    backgroundImage: "assets/images/oberyn_martell_background.jpg",
    gallery: [
      "assets/images/oberyn_1.jpg",
      "assets/images/oberyn_2.jpg",
      "assets/images/oberyn_3.jpg",
    ],
  },

  {
    code: 34,
    urlImage: "assets/images/verme_cinzento.jpg",
    alt: "Verme Cinzento, o leal aliado de Daenerys Targaryen",
    nameCharacters: "Verme Cinzento",
    origin: "Essos",
    house: "Não tem casa",
    title: "Comandante dos Imaculados",
    quote: "Apenas ouça as minhas ordens.",
    description:
      "Verme Cinzento é um dos personagens mais marcantes da série, conhecido por sua lealdade inabalável a Daenerys Targaryen. Originalmente um escravo, ele é um dos Imaculados, guerreiros treinados para serem perfeitos em combate. Com habilidades excepcionais, Verme Cinzento se destaca no campo de batalha e é um líder natural. Sua devoção a Daenerys é motivada não apenas por amor, mas também por um profundo respeito por sua luta contra a opressão. Ele simboliza a luta pela liberdade e a busca por uma nova vida em Westeros, onde ele pode finalmente ser livre.",
    backgroundImage: "assets/images/verme_cinzento_background.jpg",
    gallery: [
      "assets/images/verme_cinzento_1.jpg",
      "assets/images/verme_cinzento_2.jpg",
      "assets/images/verme_cinzento_3.jpg",
    ],
  },

  {
    code: 35,
    urlImage: "assets/images/lyanna_mormont.jpg",
    alt: "Lyanna Mormont, a jovem líder de Casa Mormont",
    nameCharacters: "Lyanna Mormont",
    origin: "Ilha de Bear",
    house: "Casa Mormont",
    title: "Lady de Casa Mormont",
    quote: "O Norte se lembra.",
    description:
      "Lyanna Mormont é uma das personagens mais impressionantes da série, conhecida por sua coragem e determinação, apesar de sua juventude. Como a Lady de Casa Mormont, ela é responsável pela liderança de sua casa e, por extensão, da ilha de Bear. Sua personalidade forte e sua habilidade em comandar são evidentes quando ela enfrenta os desafios de seu papel em meio à guerra. Lyanna se recusa a ser subestimada por causa de sua idade e demonstra uma lealdade inabalável ao Norte e a seus aliados. Sua famosa frase, 'O Norte se lembra', reflete sua determinação em honrar a história e os deveres de sua casa.",
    backgroundImage: "assets/images/lyanna_mormont_background.jpg",
    gallery: [
      "assets/images/lyanna_mormont_1.jpg",
      "assets/images/lyanna_mormont_2.jpg",
      "assets/images/lyanna_mormont_3.jpg",
    ],
  },

  {
    code: 36,
    urlImage: "assets/images/ellaria_sand.jpg",
    alt: "Ellaria Sand, a amante de Oberyn Martell",
    nameCharacters: "Ellaria Sand",
    origin: "Dorne",
    house: "Casa Martell",
    title: "Amante de Oberyn Martell",
    quote: "Eu sou o veneno que se torna o fogo.",
    description:
      "Ellaria Sand é uma das personagens mais intrigantes de 'Game of Thrones', conhecida por sua beleza e ferocidade. Como a amante de Oberyn Martell, ela desempenha um papel importante nas dinâmicas políticas de Dorne e é leal à sua família, especialmente a seus filhos. Sua paixão por Oberyn é notável, e sua determinação em proteger seus entes queridos a torna uma aliada formidável e uma adversária a ser respeitada. Ellaria não hesita em desafiar aqueles que ameaçam sua família e suas crenças, refletindo a natureza ardente de seu caráter.",
    backgroundImage: "assets/images/ellaria_sand_background.jpg",
    gallery: [
      "assets/images/ellaria_sand_1.jpg",
      "assets/images/ellaria_sand_2.jpg",
      "assets/images/ellaria_sand_3.jpg",
    ],
  },

  {
    code: 37,
    urlImage: "assets/images/yara_greyjoy.jpg",
    alt: "Yara Greyjoy, a destemida filha de Balon Greyjoy",
    nameCharacters: "Yara Greyjoy",
    origin: "Ilhas de Ferro",
    house: "Casa Greyjoy",
    title: "Filha de Balon Greyjoy",
    quote: "Eu sou Yara Greyjoy. Meu pai é um rei e eu sou uma mulher.",
    description:
      "Yara Greyjoy é uma das personagens mais fortes e determinadas de 'Game of Thrones'. Filha de Balon Greyjoy, ela se destaca em um mundo dominado por homens, desafiando as normas tradicionais de gênero. Yara é uma capitã de navio habilidosa e uma líder natural, que não hesita em lutar pelo que acredita. Sua lealdade à sua família e sua busca por independência a tornam uma figura complexa, mas admirável. Ao longo da série, ela busca honrar o legado de sua família enquanto busca seu próprio caminho, tornando-se uma guerreira respeitada nas Ilhas de Ferro.",
    backgroundImage: "assets/images/yara_greyjoy_background.jpg",
    gallery: [
      "assets/images/yara_greyjoy_1.jpg",
      "assets/images/yara_greyjoy_2.jpg",
      "assets/images/yara_greyjoy_3.jpg",
    ],
  },

  {
    code: 38,
    urlImage: "assets/images/alto_pardal.jpg",
    alt: "Alto Pardal, o líder religioso dos Sparrows",
    nameCharacters: "Alto Pardal",
    origin: "King's Landing",
    house: "Sem casa",
    title: "Líder dos Sparrows",
    quote: "Apenas os humildes têm lugar no reino de Deus.",
    description:
      "Alto Pardal é uma figura intrigante em 'Game of Thrones', conhecido por sua humildade e devoção religiosa. Como líder dos Sparrows, ele promove a ideia de uma moralidade simples e uma crítica à corrupção da nobreza. Sua ascensão ao poder em King's Landing traz uma nova ordem social, mas também gera conflito com as casas nobres, especialmente a Casa Lannister. Alto Pardal é um personagem complexo que explora temas de fé, poder e hipocrisia, mostrando que a verdadeira força pode vir dos mais humildes. Sua influência sobre os eventos na capital culmina em tensões dramáticas que impactam toda a narrativa.",
    backgroundImage: "assets/images/alto_pardal_background.jpg",
    gallery: [
      "assets/images/alto_pardal_1.jpg",
      "assets/images/alto_pardal_2.jpg",
      "assets/images/alto_pardal_3.jpg",
    ],
  },

  {
    code: 39,
    urlImage: "assets/images/olenna_tyrell.jpg",
    alt: "Olenna Tyrell, a matriarca astuta da Casa Tyrell",
    nameCharacters: "Olenna Tyrell",
    origin: "Highgarden",
    house: "Tyrell",
    title: "A Rainha dos Espinhos",
    quote: "Eu não tenho paciência para homens com o coração mole.",
    description:
      "Olenna Tyrell, conhecida como a Rainha dos Espinhos, é uma das personagens mais astutas e intrigantes de 'Game of Thrones'. Ela é a matriarca da Casa Tyrell, uma das famílias mais poderosas de Westeros. Com uma mente afiada e um humor mordaz, Olenna é famosa por suas manobras políticas e por seu papel decisivo na corte. Ela valoriza a lealdade e a inteligência, usando sua sagacidade para proteger sua família e seus interesses. Seu caráter forte e suas estratégias políticas a tornam uma adversária formidável, capaz de enfrentar os mais poderosos, incluindo os Lannisters. Olenna é também uma das vozes mais memoráveis da série, frequentemente expressando suas opiniões de forma direta e incisiva.",
    backgroundImage: "assets/images/olenna_tyrell_background.jpg",
    gallery: [
      "assets/images/olenna_tyrell_1.jpg",
      "assets/images/olenna_tyrell_2.jpg",
      "assets/images/olenna_tyrell_3.jpg",
    ],
  },

  {
    code: 40,
    urlImage: "assets/images/night_king.jpg",
    alt: "Rei da Noite, o líder temível dos Caminhantes Brancos",
    nameCharacters: "Rei da Noite",
    origin: "Além da Muralha",
    house: "Caminhantes Brancos",
    title: "Líder dos Mortos",
    quote: "Você não pode lutar contra a morte.",
    description:
      "O Rei da Noite é uma das figuras mais enigmáticas e poderosas de 'Game of Thrones'. Ele é o líder dos Caminhantes Brancos, criaturas reanimadas que ameaçam a humanidade. Criado a partir de um homem vivo pela magia dos Filhos da Floresta, o Rei da Noite possui o poder de controlar os mortos e um conhecimento profundo das sombras e do frio. Sua presença na série simboliza a luta entre a vida e a morte, e seu objetivo é trazer o inverno eterno a Westeros. Através de sua conexão com a magia antiga, ele se tornou uma das maiores ameaças que os protagonistas enfrentam, estabelecendo-se como uma força aterrorizante e inevitável.",
    backgroundImage: "assets/images/night_king_background.jpg",
    gallery: [
      "assets/images/night_king_1.jpg",
      "assets/images/night_king_2.jpg",
      "assets/images/night_king_3.jpg",
    ],
  },

  {
    code: 41,
    urlImage: "assets/images/shireen_baratheon.jpg",
    alt: "Shireen Baratheon, a corajosa filha de Stannis Baratheon",
    nameCharacters: "Shireen Baratheon",
    origin: "Península de Storm's End",
    house: "Baratheon",
    title: "Princesa de Pedra",
    quote: "Eu não sou uma princesa. Eu sou uma Baratheon.",
    description:
      "Shireen Baratheon é a única filha de Stannis Baratheon e Selyse Florent. Ela é uma jovem corajosa e inteligente, marcada pela doença que a deixou com cicatrizes. Apesar de seu tratamento cruel por parte de sua mãe, Shireen é gentil e valoriza a bondade em um mundo frequentemente cruel. Ela é uma grande apoiadora da causa de seu pai, mas seu destino trágico se entrelaça com as lutas de poder na Guerra dos Tronos. Seu amor por livros e histórias a torna uma figura única, mostrando que até mesmo nas circunstâncias mais sombrias, a esperança e a coragem podem prevalecer.",
    backgroundImage: "assets/images/shireen_baratheon_background.jpg",
    gallery: [
      "assets/images/shireen_baratheon_1.jpg",
      "assets/images/shireen_baratheon_2.jpg",
      "assets/images/shireen_baratheon_3.jpg",
    ],
  },

  {
    code: 42,
    urlImage: "assets/images/brown.jpg",
    alt: "Bronn, o astuto mercenário e aliado de Tyrion Lannister",
    nameCharacters: "Bronn",
    origin: "Forte do Pavor",
    house: "N/A",
    title: "Cavaleiro do Vale",
    quote: "A vida é feita de escolhas.",
    description:
      "Bronn é um mercenário pragmático e astuto que se torna aliado de Tyrion Lannister. Ele é conhecido por sua habilidade com a espada e sua disposição para fazer o que for necessário para sobreviver. Bronn é alguém que valoriza o poder e a riqueza, e sua lealdade pode ser comprada. Apesar de sua natureza egoísta, ele demonstra momentos de amizade e camaradagem, especialmente com Tyrion, a quem protege em várias ocasiões. Sua trajetória ao longo da série reflete os conflitos entre honra, ambição e sobrevivência em Westeros.",
    backgroundImage: "assets/images/bronn_background.jpg",
    gallery: [
      "assets/images/bronn_1.jpg",
      "assets/images/bronn_2.jpg",
      "assets/images/bronn_3.jpg",
    ],
  },

  {
    code: 43,
    urlImage: "assets/images/aemon_targaryen.jpg",
    alt: "Aemon Targaryen, o Maestre da Patrulha da Noite e último Targaryen vivo",
    nameCharacters: "Aemon Targaryen",
    origin: "Casa Targaryen",
    house: "Targaryen",
    title: "Maestre da Patrulha da Noite",
    quote: "O sangue de dragão ainda corre em minhas veias.",
    description:
      "Aemon Targaryen é um membro da antiga e nobre Casa Targaryen, conhecido por seu papel como Maestre da Patrulha da Noite. Ele é um dos últimos de sua linhagem e, como tal, carrega o peso de sua herança. Apesar de sua posição na Patrulha, Aemon mantém a dignidade e a sabedoria de um Targaryen, demonstrando grande compaixão pelos homens sob seu comando. Ele tem uma profunda compreensão das questões que afligem Westeros e, muitas vezes, reflete sobre o que poderia ter sido se os Targaryen não tivessem perdido o trono. Aemon representa o legado da Casa Targaryen e a luta pela sobrevivência em um mundo que mudou drasticamente.",
    backgroundImage: "assets/images/aemon_background.jpg",
    gallery: [
      "assets/images/aemon_1.jpg",
      "assets/images/aemon_2.jpg",
      "assets/images/aemon_3.jpg",
    ],
  },

  {
    code: 44,
    urlImage: "assets/images/osha.jpg",
    alt: "Osha, a sobrevivente feroz e astuta do Norte",
    nameCharacters: "Osha",
    origin: "Norte",
    house: "N/A",
    title: "Acompanhante dos Stark",
    quote: "Acho que a liberdade é algo a se conquistar.",
    description:
      "Osha é uma mulher selvagem do Norte, conhecida por sua astúcia e habilidades de sobrevivência. Ela se destaca como uma figura forte e independente, que desafia as normas da sociedade aristocrática de Westeros. A princípio, Osha é vista como uma ameaça para a Casa Stark, mas ao longo do tempo, ela se torna uma aliada valiosa. Sua lealdade é inegável, e seu conhecimento das terras do Norte a torna uma personagem intrigante, com um passado complexo e uma determinação feroz. Osha é um símbolo de resistência e adaptabilidade em um mundo hostil.",
    backgroundImage: "assets/images/osha_background.jpg",
    gallery: [
      "assets/images/osha_1.jpg",
      "assets/images/osha_2.jpg",
      "assets/images/osha_3.jpg",
    ],
  },

  {
    code: 45,
    urlImage: "assets/images/jojen_reed.jpg",
    alt: "Jojen Reed, o jovem visionário e portador de sonhos",
    nameCharacters: "Jojen Reed",
    origin: "Norte",
    house: "Casa Reed",
    title: "Greenseer",
    quote: "Os sonhos são uma janela para o futuro.",
    description:
      "Jojen Reed é um jovem greenseer da Casa Reed, uma família antiga e leal ao Norte. Desde cedo, Jojen possui visões proféticas que o guiam em sua jornada. Ele é um personagem introspectivo e sábio, que carrega um profundo entendimento dos eventos que cercam os Stark e o destino de Westeros. Sua conexão com os sonhos e a natureza o torna uma figura essencial na luta contra a escuridão que se aproxima. Jojen é um aliado valioso para Bran Stark, ajudando-o a entender seu próprio poder e o caminho que deve seguir.",
    backgroundImage: "assets/images/jojen_reed_background.jpg",
    gallery: [
      "assets/images/jojen_reed_1.jpg",
      "assets/images/jojen_reed_2.jpg",
      "assets/images/jojen_reed_3.jpg",
    ],
  },

  {
    code: 46,
    urlImage: "assets/images/barristan_selmy.jpg",
    alt: "Barristan Selmy, o lendário cavaleiro da Guarda Real",
    nameCharacters: "Barristan Selmy",
    origin: "Reino dos Sete",
    house: "Casa Selmy",
    title: "Cavaleiro da Guarda Real",
    quote: "A honra é uma moeda de troca que vale a pena ter.",
    description:
      "Barristan Selmy é um dos cavaleiros mais respeitados de Westeros, conhecido por sua habilidade excepcional em combate e sua inabalável honra. Ele serviu como membro da Guarda Real, leal ao rei e ao seu dever. Selmy é um símbolo de bravura e lealdade, e seus feitos em batalha o tornaram uma lenda viva. Apesar de sua reputação, ele também carrega uma profunda tristeza e luto por tempos passados. Barristan se destaca por sua sabedoria e senso de justiça, sempre buscando fazer o que é certo, mesmo quando isso significa se opor a aqueles que deveriam ser seus superiores.",
    backgroundImage: "assets/images/barristan_selmy_background.jpg",
    gallery: [
      "assets/images/barristan_selmy_1.jpg",
      "assets/images/barristan_selmy_2.jpg",
      "assets/images/barristan_selmy_3.jpg",
    ],
  },

  {
    code: 47,
    urlImage: "assets/images/walder_frey.jpg",
    alt: "Walder Frey, o Senhor dos Frey",
    nameCharacters: "Walder Frey",
    origin: "Terras Fluviais",
    house: "Casa Frey",
    title: "Senhor de Vento Frio",
    quote: "A traição é um prato que se serve frio.",
    description:
      "Walder Frey é o astuto e manipulador Senhor de Vento Frio, conhecido por sua habilidade em tecer alianças e trair seus aliados. Ele é o patriarca da Casa Frey e possui uma vasta prole, sempre pronto para utilizar a força de seus filhos e filhas para garantir a segurança e o poder de sua casa. O casamento de Edmure Tully e Roslin Frey, que se tornou um evento infame, exemplifica sua falta de honra e disposição para trair aqueles que deveriam ser aliados. Frey é um personagem emblemático da política traiçoeira de Westeros.",
    backgroundImage: "assets/images/walder_frey_background.jpg",
    gallery: [
      "assets/images/walder_frey_1.jpg",
      "assets/images/walder_frey_2.jpg",
      "assets/images/walder_frey_3.jpg",
    ],
  },

  {
    code: 48,
    urlImage: "assets/images/roose_bolton.jpg",
    alt: "Roose Bolton, o Senhor de Ponta Tempestade",
    nameCharacters: "Roose Bolton",
    origin: "Terras Fluviais",
    house: "Casa Bolton",
    title: "Senhor de Ponta Tempestade",
    quote: "A sombra da noite é uma aliada fiel.",
    description:
      "Roose Bolton é o astuto e cruel Senhor de Ponta Tempestade, conhecido por sua frieza e pelo uso da brutalidade em sua busca por poder. Ele é o patriarca da Casa Bolton e notoriamente traiu os Stark, seus antigos aliados, durante a Guerra dos Cinco Reis. Com um olhar impassível e uma reputação de impiedade, Roose manipula a política de Westeros em seu benefício, revelando-se um personagem central nas intrigas da história. Sua disposição para cometer atrocidades, como o assassinato de Robb Stark, cimenta sua posição como um dos vilões mais temidos de Westeros.",
    backgroundImage: "assets/images/roose_bolton_background.jpg",
    gallery: [
      "assets/images/roose_bolton_1.jpg",
      "assets/images/roose_bolton_2.jpg",
      "assets/images/roose_bolton_3.jpg",
    ],
  },

  {
    code: 49,
    urlImage: "assets/images/olly.jpg",
    alt: "Olly, o jovem do povo livre",
    nameCharacters: "Olly",
    origin: "Muralha",
    house: "N/A",
    title: "Jovem do Povo Livre",
    quote: "Eu só quero que você volte.",
    description:
      "Olly é um jovem do povo livre que se une à Patrulha da Noite após perder sua família para os Caminhantes Brancos. Sua lealdade e coragem são evidentes, mas sua experiência o torna cínico em relação a aqueles que não compartilham seu sofrimento. Através de sua relação com Jon Snow, Olly representa a luta entre o dever e a amizade. Com um coração angustiado, suas ações refletem o desespero de quem busca justiça em um mundo brutal e implacável.",
    backgroundImage: "assets/images/olly_background.jpg",
    gallery: [
      "assets/images/olly_1.jpg",
      "assets/images/olly_2.jpg",
      "assets/images/olly_3.jpg",
    ],
  },

  {
    code: 50,
    urlImage: "assets/images/alliser_thorne.jpg",
    alt: "Alliser Thorne, um dos líderes da Patrulha da Noite",
    nameCharacters: "Alliser Thorne",
    origin: "Muralha",
    house: "N/A",
    title: "Comandante da Patrulha da Noite",
    quote: "Você nunca será um verdadeiro Stark.",
    description:
      "Alliser Thorne é um membro da Patrulha da Noite, conhecido por seu comportamento rígido e sua atitude de desprezo, especialmente em relação a Jon Snow. Ele é um líder impiedoso, cujas crenças tradicionais o colocam em conflito com Jon e outros membros da Patrulha. Thorne acredita que a disciplina e a ordem são essenciais para a sobrevivência da Patrulha. No entanto, seu comportamento pode ser motivado por ciúmes e inseguranças, especialmente à medida que Jon se torna mais popular e respeitado.",
    backgroundImage: "assets/images/alliser_thorne_background.jpg",
    gallery: [
      "assets/images/alliser_thorne_1.jpg",
      "assets/images/alliser_thorne_2.jpg",
      "assets/images/alliser_thorne_3.jpg",
    ],
  },

  {
    code: 51,
    urlImage: "assets/images/jeor_mormont.jpg",
    alt: "Jeor Mormont, o Senhor Comandante da Patrulha da Noite",
    nameCharacters: "Jeor Mormont",
    origin: "Muralha",
    house: "Mormont",
    title: "Senhor Comandante da Patrulha da Noite",
    quote: "A Muralha é nosso lar.",
    description:
      "Jeor Mormont é o Senhor Comandante da Patrulha da Noite e um líder respeitado por sua bravura e sabedoria. Ele é um homem de princípios firmes, que valoriza a lealdade e a coragem em seus homens. Mormont é conhecido por sua preocupação com a segurança da Muralha e seu povo, especialmente quando ameaças como os Caminhantes Brancos se tornam mais iminentes. Ele é um mentor para Jon Snow, ensinando-lhe sobre a importância da honra e do dever na Patrulha.",
    backgroundImage: "assets/images/jeor_mormont_background.jpg",
    gallery: [
      "assets/images/jeor_mormont_1.jpg",
      "assets/images/jeor_mormont_2.jpg",
      "assets/images/jeor_mormont_3.jpg",
    ],
  },

  {
    code: 52,
    urlImage: "assets/images/viserys_targaryen.jpg",
    alt: "Viserys Targaryen, o Príncipe das Cidades",
    nameCharacters: "Viserys Targaryen",
    origin: "Essos",
    house: "Targaryen",
    title: "Príncipe de Westeros",
    quote: "Sou o legítimo herdeiro do trono de ferro.",
    description:
      "Viserys Targaryen é um personagem complexo da série, conhecido por sua obsessão em recuperar o trono de Westeros. Ele é o irmão mais novo de Daenerys e se considera o legítimo herdeiro do Trono de Ferro. Sua personalidade é marcada por um profundo ressentimento e uma necessidade de poder, levando-o a manipular sua irmã para alcançar seus próprios objetivos. Apesar de sua nobre origem, Viserys é frequentemente visto como arrogante e impiedoso, disposto a fazer qualquer coisa para garantir seu lugar no trono.",
    backgroundImage: "assets/images/viserys_targaryen_background.jpg",
    gallery: [
      "assets/images/viserys_targaryen_1.jpg",
      "assets/images/viserys_targaryen_2.jpg",
      "assets/images/viserys_targaryen_3.jpg",
    ],
  },

  {
    code: 53,
    urlImage: "assets/images/podrick_payne.jpg",
    alt: "Podrick Payne, o escudeiro leal",
    nameCharacters: "Podrick Payne",
    origin: "Westeros",
    house: "Payne",
    title: "Escudeiro de Tyrion Lannister",
    quote:
      "Eu não sou um homem de guerra, mas farei o que for preciso para proteger aqueles que amo.",
    description:
      "Podrick Payne é um escudeiro leal e corajoso, conhecido por seu serviço a Tyrion Lannister. Ele se destaca por sua fidelidade e determinação, mesmo em face de grandes perigos. Apesar de seu jeito tímido, Podrick possui habilidades notáveis, especialmente em combate e estratégia. Sua história é marcada por momentos de bravura e um profundo senso de lealdade, tornando-o um personagem querido entre os fãs da série.",
    backgroundImage: "assets/images/podrick_payne_background.jpg",
    gallery: [
      "assets/images/podrick_payne_1.jpg",
      "assets/images/podrick_payne_2.jpg",
      "assets/images/podrick_payne_3.jpg",
    ],
  },

  {
    code: 54,
    urlImage: "assets/images/meera_reed.jpg",
    alt: "Meera Reed, a corajosa e habilidosa guerreira",
    nameCharacters: "Meera Reed",
    origin: "Westeros",
    house: "Reed",
    title: "Guerreira de Winterfell",
    quote: "Não se esqueça, eu sou a que vai te salvar.",
    description:
      "Meera Reed é uma corajosa e habilidosa guerreira, filha de Howland Reed, e é uma das principais aliadas de Bran Stark em sua jornada. Conhecida por sua bravura e habilidades de combate, Meera demonstra uma lealdade inabalável e um espírito protetor, lutando ferozmente para proteger Bran e garantir sua segurança. Sua conexão com a magia e a natureza a torna uma personagem fascinante, sendo uma das últimas representantes da Casa Reed.",
    backgroundImage: "assets/images/meera_reed_background.jpg",
    gallery: [
      "assets/images/meera_reed_1.jpg",
      "assets/images/meera_reed_2.jpg",
      "assets/images/meera_reed_3.jpg",
    ],
  },

  {
    code: 55,
    urlImage: "assets/images/gendry.jpg",
    alt: "Gendry, o habilidoso ferreiro e bastardo de Robert Baratheon",
    nameCharacters: "Gendry",
    origin: "Westeros",
    house: "Baratheon",
    title: "Ferreiro e Bastardo",
    quote: "Eu sou um Baratheon. E isso significa algo.",
    description:
      "Gendry é um bastardo de Robert Baratheon e um habilidoso ferreiro, conhecido por sua força e determinação. Desde sua juventude, ele tem lutado para encontrar seu lugar no mundo e entender seu legado como filho de um rei. Gendry se destaca por sua habilidade em forjar armas, especialmente quando se trata de criar armas para a luta contra os Caminhantes Brancos. Sua lealdade a seus amigos e sua coragem em enfrentar desafios o tornam um personagem cativante na história de 'Game of Thrones'.",
    backgroundImage: "assets/images/gendry_background.jpg",
    gallery: [
      "assets/images/gendry_1.jpg",
      "assets/images/gendry_2.jpg",
      "assets/images/gendry_3.jpg",
    ],
  },

  {
    code: 56,
    urlImage: "assets/images/qyburn.jpg",
    alt: "Qyburn, o ex-mestre e especialista em medicina",
    nameCharacters: "Qyburn",
    origin: "Westeros",
    house: "Nenhuma",
    title: "Mestre e Praticante de Ciência Negra",
    quote: "A ciência é mais poderosa do que qualquer magia.",
    description:
      "Qyburn é um ex-mestre da Cidadela que se destacou por suas práticas não ortodoxas de medicina e ciência. Ele é conhecido por sua habilidade em reanimar os mortos e por seus experimentos com os limites da vida e da morte. Qyburn se tornou um aliado de Cersei Lannister, usando seu conhecimento e habilidades para ajudar a consolidar seu poder em King's Landing. Sua ambição e falta de ética o tornam um personagem intrigante e temido na série.",
    backgroundImage: "assets/images/qyburn_background.jpg",
    gallery: [
      "assets/images/qyburn_1.jpg",
      "assets/images/qyburn_2.jpg",
      "assets/images/qyburn_3.jpg",
    ],
  },

  {
    code: 57,
    urlImage: "assets/images/euron_greyjoy.jpg",
    alt: "Euron Greyjoy, o temido capitão dos Irmãos de Ferro",
    nameCharacters: "Euron Greyjoy",
    origin: "Ilhas de Ferro",
    house: "Casa Greyjoy",
    title: "Rei dos Irmãos de Ferro",
    quote: "Aquele que controla o mar, controla tudo.",
    description:
      "Euron Greyjoy é o ambicioso e traiçoeiro irmão mais novo de Balon Greyjoy. Conhecido por sua brutalidade e astúcia, ele retorna às Ilhas de Ferro após anos navegando pelos mares. Euron é um líder carismático que busca se tornar o novo Rei dos Irmãos de Ferro, desafiando a tradição e utilizando estratégias brutais para alcançar seus objetivos. Seu amor pelo caos e pela conquista o torna um adversário perigoso na luta pelo poder em Westeros.",
    backgroundImage: "assets/images/euron_greyjoy_background.jpg",
    gallery: [
      "assets/images/euron_1.jpg",
      "assets/images/euron_2.jpg",
      "assets/images/euron_3.jpg",
    ],
  },

  {
    code: 58,
    urlImage: "assets/images/gilly.jpg",
    alt: "Gilly, a corajosa mulher do Muro",
    nameCharacters: "Gilly",
    origin: "Muro",
    house: "N/A",
    title: "Membro dos homens livres",
    quote: "Eu apenas quero ser livre.",
    description:
      "Gilly é uma mulher forte e determinada que vive entre os homens livres, sendo filha de um líder de uma das tribos. Ela foi resgatada por Samwell Tarly, e sua vida mudou drasticamente ao lado dele. Gilly é conhecida por sua lealdade e coragem, enfrentando as adversidades do Norte e lutando para proteger aqueles que ama. Sua relação com Sam é central para sua história, mostrando a luta e a busca pela liberdade em um mundo cheio de opressão e medo.",
    backgroundImage: "assets/images/gilly_background.jpg",
    gallery: [
      "assets/images/gilly_1.jpg",
      "assets/images/gilly_2.jpg",
      "assets/images/gilly_3.jpg",
    ],
  },

  {
    code: 59,
    urlImage: "assets/images/robin_arryn.jpg",
    alt: "Robin Arryn, o jovem Lorde de Ninho da Águia",
    nameCharacters: "Robin Arryn",
    origin: "Ninho da Águia",
    house: "Arryn",
    title: "Lorde de Ninho da Águia",
    quote: "Eu sou o Senhor de Ninho da Águia!",
    description:
      "Robin Arryn, o jovem e frágil Lorde de Ninho da Águia, é um personagem complexo e muitas vezes visto como ingênuo. Desde a morte de sua mãe, ele ficou sob a tutela de sua mãe adotiva, que o influencia a seguir seus instintos e desejos. Robin é conhecido por seu comportamento mimado e sua conexão com sua mãe, que frequentemente o protege do mundo exterior. Sua jornada reflete a luta pelo poder e a fragilidade da liderança na luta pelo Trono de Ferro.",
    backgroundImage: "assets/images/robin_arryn_background.jpg",
    gallery: [
      "assets/images/robin_arryn_1.jpg",
      "assets/images/robin_arryn_2.jpg",
      "assets/images/robin_arryn_3.jpg",
    ],
  },

  {
    code: 60,
    urlImage: "assets/images/lysa_arryn.jpg",
    alt: "Lysa Arryn, a mãe controladora de Robin Arryn",
    nameCharacters: "Lysa Arryn",
    origin: "Ninho da Águia",
    house: "Arryn",
    title: "Lady de Ninho da Águia",
    quote: "O sangue do meu sangue!",
    description:
      "Lysa Arryn, a mãe de Robin Arryn, é uma mulher complexa marcada por traumas do passado e uma profunda possessividade em relação ao filho. Depois da morte de seu marido, Lysa se tornou uma figura protetora e controladora, temendo por sua segurança. Sua fragilidade emocional e a manipulação política a tornam um personagem intrigante. Lysa frequentemente age por impulso, guiada por suas emoções e experiências passadas, especialmente em relação a Petyr Baelish, a quem ama e odeia simultaneamente.",
    backgroundImage: "assets/images/lysa_arryn_background.jpg",
    gallery: [
      "assets/images/lysa_arryn_1.jpg",
      "assets/images/lysa_arryn_2.jpg",
      "assets/images/lysa_arryn_3.jpg",
    ],
  },

  {
    code: 61,
    urlImage: "assets/images/septa_unella.jpg",
    alt: "Septã Unella, a sacerdotisa devota e implacável",
    nameCharacters: "Septã Unella",
    origin: "Cidade de Porto Real",
    house: "Nenhum",
    title: "Sacerdotisa da Fé dos Sete",
    quote: "Confesse seus pecados.",
    description:
      "Septã Unella é uma fervorosa sacerdotisa da Fé dos Sete, conhecida por sua dedicação à sua religião e pela brutalidade com que aplica a justiça. Ela se tornou uma figura proeminente durante a ascensão do Alto Pardal e é famosa por sua implacável busca pela confissão e penitência. Quando Cersei Lannister é presa, Unella é encarregada de vigiá-la e humilhá-la, simbolizando a luta entre o poder secular e a religião. Sua natureza austera e comprometida revela a profundidade da fé que ela representa.",
    backgroundImage: "assets/images/septa_unella_background.jpg",
    gallery: [
      "assets/images/septa_unella_1.jpg",
      "assets/images/septa_unella_2.jpg",
      "assets/images/septa_unella_3.jpg",
    ],
  },

  {
    code: 62,
    urlImage: "assets/images/gregor_clegane.jpg",
    alt: "Gregor Clegane, conhecido como A Montanha",
    nameCharacters: "Gregor Clegane",
    origin: "Oeste de Westeros, nas terras da Casa Lannister",
    house: "Casa Clegane",
    title: "A Montanha",
    quote: "Sua brutalidade não conhece limites.",
    description:
      "Gregor Clegane, também conhecido como 'A Montanha', é um dos personagens mais temidos de Westeros. Enorme em estatura e temido por sua crueldade, ele é leal à Casa Lannister e serve como instrumento de violência e intimidação. Sua infâmia se consolidou durante as Guerras de Robert, quando se envolveu em massacres brutais. Além de sua força incomparável, a Montanha também é conhecida por sua lealdade incansável a Tywin Lannister e, mais tarde, a Cersei. Após ser gravemente ferido, ele é ressuscitado por Qyburn e se torna ainda mais aterrorizante, com uma força sobre-humana.",
    backgroundImage: "assets/images/gregor_clegane_background.jpg",
    gallery: [
      "assets/images/gregor_clegane_1.jpg",
      "assets/images/gregor_clegane_2.jpg",
      "assets/images/gregor_clegane_3.jpg",
    ],
  },
];
